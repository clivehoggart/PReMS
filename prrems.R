...
prrems <- function( y, x, x.fixed=NULL, max2way="all", k.max=5, omega=0.5, family='gaussian', tau=1, delta=1, max.s=10, no.cores=10, standardize=TRUE, verbose=TRUE ){
...
predict.prrems <- function( fitted.models, newx, size=NULL, rank=1, no.cores=10, criteria='waic', fit='mean', family='binomial' ){
    ptr.covs.use <- which( fitted.models$sd!=0 )
...
predict.prrems.bayes <- function( fitted.models, newx, y.train, x.train, size=NULL, rank=1, iter=10000, no.cores=10, criteria='waic' ){
    ptr.covs.use <- which( fitted.models$sd!=0 )
...
prrems.clean <- function( all.fits ){
    model.indicator <- list()
    fitted.models <- list()
...
cv.prrems <- function( y, x, no.cores=10, k.min=1, k.max, max.s=50, max2way='all', standardize=TRUE, nfolds=NULL, foldid=NULL, n.waic=10000, n.coef=1, lasso.penalty="lambda.min" ){
    ...
    for( i in 1:nfolds ){
        ...
        test <- which( foldid==i )
        tau <- TauEst( y[train], x[train,], standardize=standardize, lasso.penalty=lasso.penalty, n.coef=n.coef, nfolds=20 )
        my.fit <- prrems( y=y[train], x=x[train,], family='binomial', tau=tau$tau, k.max=k.max, max.s=max.s, standardize=standardize, max2way=max2way, no.cores=no.cores, verbose=FALSE )
...
my.auc <- function( my.fit, sizes, X, y, rank=1 ){
    ...
    for( i in sizes ){
        ii <- i - min(sizes) + 1
        my.pred[,ii] <- predict.prrems( my.fit, as.matrix(X), size=i, rank=rank, no.cores=10, criteria='waic', fit='mean', family='binomial' )
    }
